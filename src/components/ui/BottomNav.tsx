...
export default function BottomNav({ tabs, activeTab, onTabChange }: BottomNavProps): React.JSX.Element {
  return (
    <nav className="fixed bottom-0 left-0 right-0 p-2 bg-brand-secondary/80 backdrop-blur-md flex justify-around items-center z-40 shadow-[0_-4px_20px_rgba(0,0,0,0.4)]">
      {tabs.map((tab) => (
        <button
          key={tab.id}
